...
def LassoCV_():
    ...
    #Specify data
    data_train = pd.read_csv("/Users/user/Desktop/UCI_HAR_Dataset/train/X_train.csv", delim_whitespace=True, header=None)
    ...
    #selct_from_model = (LinearSVC(max_iter=100000)).fit(data_train,output_train)
    selct_from_model = (LassoCV(max_iter=100000)).fit(data_train, output_train)
    #selct_from_model = (RandomForestClassifier()).fit(data_train, output_train)
    model = SelectFromModel(selct_from_model, prefit=True)
    ...
    print("Data_train", np.shape(data_train), "Data_test", np.shape(data_test), "Output_train",np.shape(output_train),
          "Output_test", np.shape(output_test), "activities_labels", np.shape(activities_labels),"groups_at_training", np.shape(groups_at_training), "groups_at_testing", np.shape(groups_at_testing))
